...
import {
    getInstances, createInstance, connectInstance, disconnectInstance, deleteInstance,
    getChats, getMessages, sendMessage, setWebhook, sendMedia, sendContact, sendLocation,
    sendPresence, markChatAsRead, reactToMessage, deleteMessage, formatMessagePreview
} from '../../services/whatsappService';
import { WhatsAppInstance, WhatsAppChat as WhatsAppChatType, WhatsAppMessage, WhatsAppMediaType } from '../../types';
import { MessageSquare, Plus, RefreshCw, Smartphone, QrCode, Power, Trash2, Search, Send, Paperclip, MoreVertical, X, Check, CheckCheck, Globe, Image, FileText, Mic, MapPin, User, Smile } from 'lucide-react';
...
export const WhatsAppChat: React.FC = () => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        // Send typing indicator (non-blocking, don't await)
        sendPresence(selectedInstance.token, selectedChat.id, 'composing', 2000).catch(() => { });

        const result = await sendMessage(selectedInstance.token, selectedChat.id, newMessageText);
        if (result) {
            setNewMessageText('');
            loadMessages(selectedInstance.token, selectedChat.id);
        } else {
    ...
    const handleSendMedia = async (file: File, type: WhatsAppMediaType) => {
        ...
        reader.onload = async () => {
            ...
            const result = await sendMedia(selectedInstance.token, {
    ...
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, type: WhatsAppMediaType) => {
    ...
    const handleReaction = async (messageId: string, emoji: string) => {
        if (!selectedInstance || !selectedChat) return;
        await reactToMessage(selectedInstance.token, selectedChat.id, messageId, emoji);
    ...
    const handleDeleteMessage = async (messageId: string) => {
        ...
        if (window.confirm('Apagar esta mensagem para todos?')) {
            await deleteMessage(selectedInstance.token, selectedChat.id, messageId, true);
            loadMessages(selectedInstance.token, selectedChat.id);
        }
        setSelectedMessageForAction(null);
    };
    ...
    const handleTyping = async () => {
        if (!selectedInstance || !selectedChat || isTyping) return;
        setIsTyping(true);
        await sendPresence(selectedInstance.token, selectedChat.id, 'composing', 3000);
                        ...
                        <div className="flex-1 flex flex-col bg-[#efeae2]">
                            {selectedChat ? (
                                <>
                                                ...
                                                <div key={msg.id} className={`flex ${msg.fromMe ? 'justify-end' : 'justify-start'} group`}>
                                                    <div className={`max-w-[70%] p-2 rounded-lg shadow-sm text-sm relative ${msg.fromMe ? 'bg-[#d9fdd3] text-slate-900 rounded-tr-none' : 'bg-white text-slate-900 rounded-tl-none'}`}>
                                                        ...
                                                        {msg.type === 'image' && msg.fileUrl && (
                                                            <img src={msg.fileUrl} alt="" className="max-w-full rounded mb-1" />
                                                        )}
                                                        {msg.type === 'location' && (
                                                            <div className="flex items-center gap-2 text-blue-600">
                                                                <MapPin size={16} />
                                                                <span>{msg.locationName || 'Localização'}</span>
                                                            </div>
                                                        )}
                                                        {msg.type === 'contact' && (
                                                            <div className="flex items-center gap-2 text-purple-600">
                                                                <User size={16} />
                                                                <span>{msg.contactName || 'Contato'}</span>
                                                            </div>
                                                        )}
                                                        <p className="whitespace-pre-wrap">{formatMessagePreview(msg)}</p>

                                                        {/* Reaction button */}
                                                        <button
                                                            onClick={() => setSelectedMessageForAction(selectedMessageForAction?.id === msg.id ? null : msg)}
                                                        ...
                                                        {/* Reaction picker */}
                                                        {selectedMessageForAction?.id === msg.id && (
                                                            <div className="absolute -top-10 right-0 bg-white rounded-full shadow-lg px-2 py-1 flex gap-1 z-10">
                                                                {['👍', '❤️', '😂', '😮', '😢', '🙏'].map(emoji => (
                                    ...
                                    {/* Input Area */}
                                    <form onSubmit={handleSendMessage} className="bg-gray-100 p-3 flex items-center gap-2 relative">
                                        {/* Attach Button with Menu */}
                                        <div className="relative">
                                            <button
                                                type="button"
                                                onClick={() => setShowAttachMenu(!showAttachMenu)}
                                                className="p-2 text-slate-500 hover:bg-gray-200 rounded-full transition-colors"
                                            >
                                                <Paperclip size={24} />
                                            </button>

                                            {showAttachMenu && (
                                                <div className="absolute bottom-12 left-0 bg-white rounded-xl shadow-xl p-2 flex flex-col gap-1 min-w-[160px] z-20">
                                                    <label className="flex items-center gap-3 px-3 py-2 hover:bg-purple-50 rounded-lg cursor-pointer transition-colors">
                                                    ...
                                                    <label className="flex items-center gap-3 px-3 py-2 hover:bg-blue-50 rounded-lg cursor-pointer transition-colors">
                                                        <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center">
                                                            <FileText size={16} className="text-white" />
                                                        </div>
                                                        <span className="text-sm font-medium">Documento</span>
                                                        <input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt" className="hidden" onChange={(e) => handleFileSelect(e, 'document')} />
                                                    </label>
                                                    <label className="flex items-center gap-3 px-3 py-2 hover:bg-orange-50 rounded-lg cursor-pointer transition-colors">
